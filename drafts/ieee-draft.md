...
Calibration and safety claims for agent-authored controllers are routinely demonstrated with a single execution. We argue that such claims are **2-safety hyperproperties**: the bad thing is not one run, but a **coupled pair** that shares all exogenous inputs except one controlled perturbation. **DIPTYCH** is an IEEE-oriented artifact harness that forks a shared prefix (open-loop shape or common-random-numbers closed-loop point), discards incomparable pairs, and grades suffixes only. It ships schema-`0.2` probe envelopes, eight operators spanning open-loop and CRN closed-loop regimes, and a `gate_axis_mutate` power-on-axis check that rejects cosmetic verdict flips. Product adapters (ZeroDay, AOMB) emit conforming/violating twins at pinned SHAs; DIPTYCH grades and does not vendor those trees. Live offline evaluation on this repository reports all eight `diptych_core` coverage cells **green** with `axis_power: true`; adapter columns remain **pending** until product emitters pass the same gates. We invent no AUROC or model scores—only repo and CI facts.
...
**Non-claims (stated up front).** Localization ≠ exploitability. No exploit/PoC payloads. No AUROC / model-score fields in graded envelopes. Adapter columns are not green until emitters pass—pending is honest.
...
- Green adapter cells without emitter evidence  
...
Artifact tip evaluated for this draft’s live matrix: repository `pandeyaby/DIPTYCH`, schema `0.2`, source row `diptych_core`. Matrix path: [`coverage/matrix.json`](../coverage/matrix.json). Notes field in that file: *diptych_core=green requires twin conf/viol AND gate_axis_mutate power-on-axis; adapter columns turn green when ZeroDay@fb5b39da / AOMB@667e475 emitters pass*.

### B. Live coverage matrix (verbatim from `coverage/matrix.json`)

Operator × `{diptych_core, zeroday, aomb}` with `axis_power`:

| Operator | diptych_core | zeroday | aomb | axis_power |
|---|---|---|---|---|
| FREEZEDRY | **green** | pending | pending | true |
| RESEED | **green** | pending | pending | true |
| SCHEMAX | **green** | pending | pending | true |
| SIGNFLIP | **green** | pending | pending | true |
| SATEXTEND | **green** | pending | pending | true |
| HISTSWAP | **green** | pending | pending | true |
| TRAJSWAP | **green** | pending | pending | true |
| VARSCALE | **green** | pending | pending | true |

**Summary:** live **green×8** on `diptych_core` with axis power on every operator; **pending×8** on both `zeroday` and `aomb` columns. No fabricated adapter greens. No AUROC column exists.
...
**External.** Adapter columns remain pending until ZeroDay@`fb5b39da` and AOMB@`667e475` emitters pass the same gates. Results on `diptych_core` do not automatically transfer to product CI mirrors.

**Conclusion validity.** We do not claim AUROC improvements, ranking lifts, or security exploit findings. Claims are limited to harness behavior and the live matrix cells cited above.
...
DIPTYCH treats calibration of agent-authored controllers as a **2-safety** grading problem over coupled traces. The harness couples prefixes, applies one of eight operators, requires asymmetric twin verdicts, and demands `gate_axis_mutate` power-on-axis. Live offline evaluation paints **all eight** `diptych_core` cells green with axis power; ZeroDay and AOMB columns stay pending at the pinned SHAs until product emitters clear the same bar. One film can be legal; only the pair is calibrated.
